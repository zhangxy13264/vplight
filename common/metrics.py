...
class Metrics(object):
    ...
    def update(self, rewards=None):
        ...
        # pedestrian change
        if 'pedestrian_delay' in self.lane_metrics.keys():
            self.lane_metrics['pedestrian_delay'] += (np.stack(np.array(
                [ag.get_pedestrian_delay() for ag in self.agents], dtype=np.float32))).flatten()
        if 'pedestrian_queue' in self.lane_metrics.keys():
            self.lane_metrics['pedestrian_queue'] += (np.stack(np.array(
                [ag.get_pedestrian_queue() for ag in self.agents], dtype=np.float32))).flatten()
    ...
    def pedestrian_queue(self):
        try:
            result = self.lane_metrics['pedestrian_queue']
            return np.sum(result) / (self.decision_num * len(self.world.intersections))
        except KeyError:
            print('pedestrian_queue in not recorded in lane_metrics, please add it into the list')
            return None
        
    def pedestrian_delay(self):
        # real_delay
        if 'pedestrian_delay' not in self.lane_metrics.keys():
            return self.world.get_real_pedestrian_delay()
        
        # apx_delay
        else:
            try:
                result = self.lane_metrics['pedestrian_delay']
                return np.sum(result) / (self.decision_num * len(self.world.intersections))
            except KeyError:
                print(('apx delay is not recorded in lane_metrics, please add it into the list'))
                return None
    
    def pedestrian_throughput(self):
        return self.world.get_cur_pedestrian_throughput()
    
    def real_average_pedestrian_travel_time(self):
        return self.world.get_average_pedestrian_travel_time()

    def combine_queue(self):
        return self.queue() + self.pedestrian_queue()
    
    def combine_delay(self):
        return self.delay() + self.pedestrian_delay()
    
    def combine_throughput(self):
        return self.throughput() + self.pedestrian_throughput()
    
    def combine_average_travel_time(self):
        return self.world.get_combine_average_travel_time()
