...
class FixedTimeAgent(BaseAgent):
    ...
    def get_pedestrian_queue(self):
        pedestrian_queue = []
        pedestrian_queue.append(self.pedestrian_queue.generate())
        pedestrian_queue = np.sum(np.squeeze(np.array(pedestrian_queue)))
        return pedestrian_queue

    def get_pedestrian_delay(self):
        delay = []
        delay.append(self.pedestrian_delay.generate())
        delay = np.sum(np.squeeze(np.array(delay)))
        return delay