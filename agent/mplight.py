...
import gym
from generator import LaneVehicleGenerator, IntersectionPhaseGenerator, IntersectionVehicleGenerator, SidewalkPedestrianGenerator
...
class MPLightAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        for inter in self.world.intersections:
            ...
            tmp_p_generator = SidewalkPedestrianGenerator(self.world, inter, ['sidewalk_waiting_count'], in_only=True, average='road', negative=True)
            rewarding_generators.append((node_idx, tmp_generator, tmp_p_generator))
        ...
        for inter in self.world.intersections:
            ...
            node_idx = self.world.id2idx[node_id]
            node_obj = self.world.id2intersection[node_id]
            tmp_generator = SidewalkPedestrianGenerator(self.world, node_obj, ["sidewalk_waiting_count"], 
                                                 in_only=True, negative=False)
            pedestrian_queues.append((node_idx, tmp_generator))
        sorted(pedestrian_queues, key=lambda x: x[0])
        self.pedestrian_queue = pedestrian_queues
        ...
        for inter in self.world.intersections:
            ...
            node_idx = self.world.id2idx[node_id]
            node_obj = self.world.id2intersection[node_id]
            tmp_generator = SidewalkPedestrianGenerator(self.world, node_obj, ["sidewalk_delay"], 
                                                         in_only=True, average=None, negative=False)
            pedestrian_delays.append((node_idx, tmp_generator))
        sorted(pedestrian_delays, key=lambda x: x[0])
        self.pedestrian_delay = pedestrian_delays
    ...
    def get_reward(self):
        ...
        if self.rank == 0:
            ...
            for i in range(len(self.reward_generator)):
                tmp_v = self.reward_generator[i][1].generate()
                tmp_p = self.reward_generator[i][2].generate()
                tmp = np.concatenate([tmp_v, tmp_p])
                tmp = np.sum(tmp)
                rewards.append(tmp)
            rewards = np.squeeze(np.array(rewards))
            self.world.agent0_rewards = rewards
        else:
            rewards = self.world.agent0_rewards
        return rewards
    ...
    def get_pedestrian_queue(self):
        if self.rank == 0:
            pedestrian_queue = []
            for i in range(len(self.pedestrian_queue)):
                pedestrian_queue.append((self.pedestrian_queue[i][1].generate()))
            try:
                tmp_pedestrian_queue = np.squeeze(np.array(pedestrian_queue))
            except: 
                tmp_pedestrian_queue = pedestrian_queue
            if self.sub_agents == 1:
                pedestrian_queue = np.sum(tmp_pedestrian_queue)
            else:
                pedestrian_queue = [np.sum(x) for x in tmp_pedestrian_queue]
            self.world.agent0_pedestrian_queue = pedestrian_queue
        else:
            pedestrian_queue = self.world.agent0_pedestrian_queue
        return pedestrian_queue
    
    def get_pedestrian_delay(self):
        if self.rank == 0:
            pedestrian_delay = []
            for i in range(len(self.pedestrian_delay)):
                pedestrian_delay.append((self.pedestrian_delay[i][1].generate()))
            pedestrian_delay = np.squeeze(np.array(pedestrian_delay))
            pedestrian_delay = np.mean(pedestrian_delay)
            self.world.agent0_pedestrian_delay = pedestrian_delay
        else:
            pedestrian_delay = self.world.agent0_pedestrian_delay
